...
\item[Description]\AsIs{An MCMC algorithm for simultaneous feature selection and classification,
...
\item[Depends]\AsIs{R (>= 2.10), DiagrammeR}
\item[Imports]\AsIs{Rcpp (>= 0.12.2), Matrix, discretization}
...
Other analysis, e.g. feature selection plots using \code{signal\_var\_proportion} (based on how often each variable appeared in the signal group).
\end{Details}
%
\begin{Author}\relax
Viktoriya Krakovna
...
\end{Author}
\inputencoding{utf8}
\HeaderA{corral\_augmented}{Augmented corral data set: synthetic data with correlated attributes augmented with noise features}{corral.Rul.augmented}
...
\end{Examples}
\inputencoding{utf8}
\HeaderA{data\_disc}{Data set discretization and formatting}{data.Rul.disc}
%
\begin{Description}\relax
...
\end{Arguments}
\inputencoding{utf8}
\HeaderA{heart}{Heart disease data set: disease outcomes given health attributes}{heart}
...
\end{Arguments}
\inputencoding{utf8}
\HeaderA{madelon}{Madelon data set: synthetic data from NIPS 2003 feature selection challenge}{madelon}
...
\end{References}
\inputencoding{utf8}
\HeaderA{sbfc}{Selective Bayesian Forest Classifier (SBFC) algorithm}{sbfc}
%
\begin{Description}\relax
...
\item[Parameters] \code{nstep}, \code{thin}, \code{burnin\_denom}, \code{cv}, \code{thinoutputs}, \code{alpha}, \code{y\_penalty}, \code{x\_penalty}.
